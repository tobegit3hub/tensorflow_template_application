...
def predict(request):
    if request.method == 'POST':
         ...
         print("Inference data: {}, inference result: {}".format(inference_data, inference_result))

         return HttpResponse("Success to predict cancer, result: {}".format(inference_result))
    else:
...
def online_train(request):
    if request.method == 'POST':
        ...
        _, loss_value, epoch = sess.run([train_op2, loss2, global_step], feed_dict={batch_features: feed_batch_features, batch_labels: feed_batch_labels})
